...
async function gfetch(url: string, options: any = {}, env: any) {
  ...
  const getHeaders = (tokenValue: string) => {
    const h: Record<string, string> = { 'Authorization': `Bearer ${tokenValue}`, ...options.headers };
...
/**
 * Lưu hoặc cập nhật file nội dung Template trong Drive (Tránh giới hạn 50k ký tự của Sheet)
 */
export const updateTemplateFile = async (env: any, folderId: string, content: string) => {
  ...
  const list = await gfetch(`${DRIVE_API_BASE}/files?q=${encodeURIComponent(q)}`, { method: 'GET' }, env);
  const existingFile = list.files?.[0];

  const metadata = { name: 'PROJECT_TEMPLATE.txt', parents: existingFile ? undefined : [folderId] };
  ...
  return await gfetch(url, { method: existingFile ? 'PATCH' : 'POST', body: form }, env);
};

/**
 * Đọc nội dung file Template từ Drive
 */
const fetchTemplateContent = async (env: any, folderId: string) => {
  const q = `'${folderId}' in parents and name = 'PROJECT_TEMPLATE.txt' and trashed = false`;
  const list = await gfetch(`${DRIVE_API_BASE}/files?q=${encodeURIComponent(q)}`, { method: 'GET' }, env);
  ...
  const res = await fetch(`${DRIVE_API_BASE}/files/${file.id}?alt=media`, {
    headers: { 'Authorization': `Bearer ${currentAccessToken || env.GOOGLE_ACCESS_TOKEN}` }
  });
  return res.text();
};
...
export const listRemoteProjectFolders = async (env: any) => {
  ...
  return (await gfetch(`${DRIVE_API_BASE}/files?q=${encodeURIComponent(q)}&fields=files(id, name)`, { method: 'GET' }, env)).files || [];
};
...
export const findProjectSheetInFolder = async (env: any, folderId: string) => {
  ...
  return (await gfetch(`${DRIVE_API_BASE}/files?q=${encodeURIComponent(q)}&fields=files(id, name)`, { method: 'GET' }, env)).files?.[0] || null;
};
...
export const fetchProjectFromSheet = async (env: any, sheetId: string, folderId: string) => {
  ...
  // Thử lấy template từ file text trước, nếu không có mới lấy từ sheet (để tương thích ngược)
  let template = await fetchTemplateContent(env, folderId);
  if (!template) template = rows[5]?.[1] || '';
...
export const createProjectStructure = async (env: any, projectName: string, parentFolderId: string = 'root') => {
  const folder = await gfetch(`${DRIVE_API_BASE}/files`, {
    method: 'POST',
    body: JSON.stringify({ name: `[API_DOC] ${projectName}`, mimeType: 'application/vnd.google-apps.folder', parents: [parentFolderId || 'root'] })
  }, env);

  const sheet = await gfetch(`${DRIVE_API_BASE}/files`, {
    method: 'POST',
    body: JSON.stringify({ name: `DB_${projectName}`, mimeType: 'application/vnd.google-apps.spreadsheet', parents: [folder.id] })
  }, env);
...
export const syncProjectToSheet = async (env: any, sheetId: string, project: any) => {
  // Đồng thời lưu template vào file riêng
  ...
  const values = [
    ...
    ["Template Ref", "Stored in PROJECT_TEMPLATE.txt"], // Không lưu text dài vào đây nữa
    [],
  ...
  project.apis.forEach((api: any) => {
    values.push([api.id, api.name, api.method, api.endpoint, api.description, api.authType, JSON.stringify(api.inputParams), JSON.stringify(api.outputParams)]);
  });

  return await gfetch(`${SHEETS_API_BASE}/${sheetId}/values/A1:H500?valueInputOption=USER_ENTERED`, {
    method: 'PUT',
...
export const uploadDocFile = async (env: any, folderId: string, fileName: string, htmlContent: string) => {
  ...
  return await gfetch(`${UPLOAD_API_BASE}/files?uploadType=multipart`, { method: 'POST', body: form }, env);
};

export const uploadRawFile = async (env: any, folderId: string, file: File) => {
  const metadata = { name: `RAW_SOURCE_${file.name}`, parents: [folderId] };
  const form = new FormData();
  form.append('metadata', new Blob([JSON.stringify(metadata)], { type: 'application/json' }));
...
export const uploadImageFile = async (env: any, folderId: string, file: File, apiName: string) => {
  ...
  const metadata = { name: `SD_${cleanApiName}_${file.name}`, parents: [folderId] };
  const form = new FormData();
  form.append('metadata', new Blob([JSON.stringify(metadata)], { type: 'application/json' }));
  form.append('file', file);
  return await gfetch(`${UPLOAD_API_BASE}/files?uploadType=multipart`, { method: 'POST', body: form }, env);
